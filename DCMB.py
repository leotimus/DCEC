...
class DCMB(object):
    def __init__(self, filters=[32, 64, 128, 60, 256], n_clusters=60, contig_len=20000):

        super(DCMB, self).__init__()
        ...
        self.cae = CAE(filters=filters, contig_len=contig_len)
    ...
    def fit(self, x, y=None, batch_size=256, maxiter=2e4, tol=1e-3,
        ...
        kmeans = KMeans(n_clusters=self.n_clusters, n_init=60)
        ...
        logfile = open(save_dir + '/dcmb_log.csv', 'w')
        ...
        for ite in range(int(maxiter)):
            ...
            if ite != 0 and ite % save_interval == 0:
                # save DCMB model checkpoints
                logdelta.write('observed losses: ' + str(loss) + '\n')
                file = save_dir + '/dcmb_model_' + str(ite) + '.h5'
        ...
        logfile.close()
        print('saving model to:', save_dir + '/dcmb_model_final.h5')
        self.model.save_weights(save_dir + '/dcmb_model_final.h5')
        t2 = time()
...
if __name__ == "__main__":
    ...
    # prepare the DCMB model
    dcmb = DCMB(filters=[32, 64, 128, 60, 256], n_clusters=args.n_clusters, contig_len=args.contig_len)
    keras.utils.plot_model(dcmb.model, to_file=args.save_dir + '/dcmb_model.png', show_shapes=True)
    dcmb.model.summary()

    # begin clustering.
    optimizer = 'adam'
    dcmb.compile(loss=['kld', tf.keras.losses.CosineSimilarity()], loss_weights=[args.gamma, 1], optimizer=optimizer)
    # Step 1: pretrain if necessary
    dcmb.init_cae(batch_size=args.batch_size, cae_weights=args.cae_weights, save_dir=args.save_dir, x=x)
    # Step 2: train with cpu
    with tf.device('/cpu:0'):
        dcmb.fit(x, y=y, tol=args.tol, maxiter=args.maxiter, update_interval=args.update_interval, save_dir=args.save_dir,
