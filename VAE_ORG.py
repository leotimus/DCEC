...
class VAE_ORG(object):

    def __init__(self, batch_size=100, n_epoch=100, n_hidden=256, z_dim=10, input_shape=None,
                 print_model=True, save_dir='results/'):
        super(VAE_ORG, self).__init__()
        ...
        inputs, self.encoder, self.z_mean, self.z_log_var = self.create_encoder(input_shape)
        self.decoder = self.decoder_model(self.encoder, input_shape)
    ...
    def create_encoder(self, input_shape) -> (Input, Model, Dense, Dense):
        ...
        z_mean = Dense(self.z_dim, name='z_mean')(x)
        z_log_var = Dense(self.z_dim, name='z_log_var')(x)
        z = Lambda(self.sampling, output_shape=(self.z_dim,), name='z')([z_mean, z_log_var])
        encoder = Model(inputs, [z_mean, z_log_var, z], name='encoder')
        if self.print_model:
            encoder.summary()
        tf.keras.utils.plot_model(encoder, to_file=f'{self.save_dir}vae_mlp_encoder.png', show_shapes=True)
    ...
    def decoder_model(self, encoder, input_shape):
        ...
        if self.print_model:
            encoder.summary()
        tf.keras.utils.plot_model(decoder, to_file=f'{self.save_dir}vae_mlp_decoder.png', show_shapes=True)
        return decoder