...
def CAE(input_shape=(1000, 6, 1), filters=[32, 64, 128, 10]):
    ...
    model.add(keras.layers.Conv2D(filters[0], 5, strides=(2,5), padding='same', activation='relu', name='conv1', input_shape=input_shape))

    model.add(keras.layers.Conv2D(filters[1], 5, strides=(2,1), padding='same', activation='relu', name='conv2'))
    ...
    model.add(keras.layers.Dense(units=filters[2]*int(input_shape[0]/8)*int(input_shape[1]/5), activation='relu'))

    model.add(keras.layers.Reshape((int(input_shape[0]/8), int(input_shape[1]/5), filters[2])))
    model.add(keras.layers.Conv2DTranspose(filters[1], 3, strides=(2,1), padding=pad3, activation='relu', name='deconv3'))

    model.add(keras.layers.Conv2DTranspose(filters[0], 5, strides=(2,1), padding='same', activation='relu', name='deconv2'))

    model.add(keras.layers.Conv2DTranspose(input_shape[2], 5, strides=(2,5), padding='same', name='deconv1'))
    model.summary()
    return model
