...
def run_tsne():
    ...
    filename = f'{save_dir}/vae_mean.png'
    # display a 2D plot of the digit classes in the latent space
    #z = dvmb.vae.encoder.predict(x, batch_size=batch_size)
    clusters = dvmb.predict(x, batch_size=batch_size)
    ...
    #Path(f'{save_dir}/bins').mkdir(exist_ok=True)
    #writeBins(f'{save_dir}/bins', bins=binsDict, fastadict=fastaDict)
    X_embedded = TSNE(n_components=2, learning_rate='auto', init='random').fit_transform(dvmb.z)
    fig, ax = plt.subplots()
