...
# def CAE1(filters=[32, 64, 128, 60], contig_len=1000):
...
def CAE2(filters=[32, 64, 128, 60], contig_len=128):
    ...
    model.add(keras.layers.Flatten())
    model.add(keras.layers.Dense(units=filters[3], name='embedding'))
    model.add(keras.layers.Dense(units=filters[2]*int(input_shape[0]/8), activation='relu'))
    model.summary()
...
if __name__ == "__main__":
    ...
    # define the model
    model = CAE2(filters=[32, 64, 128, 60], contig_len=20000)
