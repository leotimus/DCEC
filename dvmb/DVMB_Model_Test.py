...
from dvmb.DVMB_Model import DVMB_Model
...
def run_deep_clustering():
    ...
    dvmb: DVMB_Model = DVMB_Model(n_hidden=n_hidden, batch_size=batch_size, n_epoch=n_epoch, n_clusters=n_clusters,
                                  save_dir=save_dir)
    dvmb.compile(optimizer='Adam')

    # pre-training
    dvmb.init_vae(x=x, epoch=100)
